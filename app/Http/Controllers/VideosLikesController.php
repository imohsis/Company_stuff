...
class VideosLikesController extends Controller
{
    ...
    public function create(Request $request){
        ...
        if(Utility::hasAccess($params->data["user"])){
            $model = new VideosLikes();

            $this->responseCode = Utility::RESPONSE_CODE_CREATED;
            $userLiked = $model->getUserLiked('video_uid',$params->data['post_uid']);
            if(count($userLiked) > 0) {
                $deleteLiked = $model->deleteLiked('video_uid',$params->data['post_uid']);
                $this->responseMessage = "dislike";
                $this->responseData = array(
                    "like" => 'deleted'
                );
            }else{
                $data =  $model->like('video_uid',$params->data['post_uid'],$params->data['type']);
